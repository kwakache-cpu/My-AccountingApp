...
| Safe connection probe framework | **YELLOW** | Phase 5B.14F adds opt-in `postgres_connection_probe.py` and `postgres_staging_deployer.py --probe`; it can only attempt connect/disconnect when `ERP_ENABLE_POSTGRES_PROBE=1` is set and never deploys schema, runs migrations, executes SQL, or enables runtime |
