...
| Blockers | Bill≠receive training mitigated by drift monitor; POS permission model **resolved (Sprint B P0-1)** |
...
**Why not unconditional:** Executed human UAT still open; drift monitor requires operators to link bill numbers on receive.

---

### Pilot condition update (Sprint B P0-2)
...
| EKA fit | Role matrix exists; POS posting permission hardened (Sprint B P0-1) |
...
| Minimum bar | Browser UAT all roles; Postgres write certification; backup rehearsal |

**Why no:** At 20 people, **concurrent POS**, and **multi-user inventory** are non-negotiable without Postgres and load certification.

**Path to GO:** 90-day remediation on P1-1 (bill/inventory auto-link), P1-2 (POS movements), Postgres pilot, executed UAT.
...
|-----------|--------------|
| **Business owner** | Accepts bill≠receive workflow; uses drift monitor; links receive references to bill numbers |
...
6. **Purchase-to-pay drift visibility** — drift monitor live; functional auto-link still P1-1.
