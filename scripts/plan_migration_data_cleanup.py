        ...
        if valid and status == "already_iso":
            items.append(
                CleanupItem(
                    ...
                    label=label,
                    current_value=current,
                    recommended_fix="No change — date is already valid ISO YYYY-MM-DD (audit false positive from GLOB `_` pattern).",
                    proposed_sql="",
                    risk_level="LOW",
                    auto_fix_safe=False,
                    manual_decision_needed=False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.apply:
        print(
            "ERROR: --apply is not implemented in Phase 5B.3. "
            "This script is read-only and prints proposed SQL only.",
            file=sys.stderr,
        )
        return 2
